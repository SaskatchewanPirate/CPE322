# Django
## stevens
### Entering a new temperature reading from the admin page
![DjangoAdmin](DjangoAdmin.png)
### Viewing the app on localhost
![DjangoLocalhost](DjangoLocalhost.png)
### Viewing the app from a separate device
![DjangoSiteView](DjangoSiteView.png)
## mycpu
### Entering a new location from the admin page
![DjangoMyCPUAdmin](DjangoMyCPUAdmin.png)
### Viewing the app on localhost
![DjangoMyCPULocalhost](DjangoMyCPULocalhost.png)
### Viewing the app from a separate device
![DjangoMyCPUSiteView](DjangoMyCPUSiteView.png)
# Flask
## hello_world
![FlaskHelloWorld](FlaskHelloWorld.png)
